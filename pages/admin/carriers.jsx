...
import { FaPlus, FaSearch, FaEdit, FaTrash, FaUserShield, FaIdCard, FaTruck, FaTimes, FaCheck, FaMapMarkerAlt, FaPhone, FaEnvelope, FaCalendarAlt, FaExclamationTriangle, FaBuilding, FaFileAlt, FaEye, FaExclamationCircle, FaTags, FaRegCalendarAlt, FaIndustry, FaIdBadge, FaUser, FaSpinner } from 'react-icons/fa'
  ...
  useEffect(() => {
    const fetchCarriers = async () => {
      try {
        ...
        // API'den taşıyıcıları çek
        const statusParam = selectedTab === 'all' ? '' : 
                            selectedTab === 'active' ? 'active' :
                            selectedTab === 'passive' ? 'inactive' :
                            selectedTab === 'pendingdocs' ? 'pendingdocs' : '';
        
        const response = await axios.get(`/api/admin/carriers`, {
          headers: {
        ...
        <div className="flex flex-row gap-4 mb-8">
          ...
          <div className="bg-white rounded-lg shadow p-4 flex-1">
            <div className="flex items-center">
              ...
              <div>
                <h3 className="text-gray-500 text-sm">Aktif Taşıyıcılar</h3>
                <p className="text-2xl font-bold">{carriers.filter(c => c.status === 'Aktif').length}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-4 flex-1">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{carriers.filter(c => c.status === 'Onay Bekliyor').length}</p>
              </div>
            </div>
          </div>
        </div>
