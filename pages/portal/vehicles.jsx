...
export default function Vehicles() {
  ...
  const handleAddVehicle = async (e) => {
    ...
    try {
      ...
      Object.keys(vehicleForm).forEach(key => {
        if (key === 'documents' && vehicleForm[key]) {
          vehicleForm[key].forEach(file => {
            formDataToSend.append('documents', file);
          });
        } else {
          formDataToSend.append(key, vehicleForm[key]);
        }
      });
  ...
  const handleEditVehicle = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`/api/portal/vehicles/${selectedVehicle._id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`
        },
  ...
  const handleDeleteVehicle = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`/api/portal/vehicles/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
